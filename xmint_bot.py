...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            results = []
            for i in range(40):  # 25 number + 15 OTP
...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    ...
    try:
        ...
        if res.status_code == 403:
            logging.warning("⚠️ S2 number: 403 — session returned")
...
# =============================================
#   OTP EXTRACTION (space-separated + fake ignore)
# =============================================
...
def extract_otp(message):
    if not message:
        return None
    m = re.search(r"\b(\d{3})\s(\d{3})\b", message)
    if m:
        otp = m.group(1) + m.group(2)
        if otp not in _FAKE_OTPS:
            return otp
    m = re.search(r"\b(\d{4})\s(\d{4})\b", message)
    if m:
        otp = m.group(1) + m.group(2)
        if otp not in _FAKE_OTPS:
            return otp
    m = re.search(r"\b(\d{8}|\d{6}|\d{5}|\d{4})\b", message)
    if m:
        otp = m.group(1)
        if otp not in _FAKE_OTPS:
            return otp
...
def extract_otp_from_record(n):
    ...
    otp = extract_otp(raw_sms) if raw_sms else None
    if otp:
        return otp, raw_sms
    raw_otp_field = (n.get("otp") or "").strip()
    otp = extract_otp(raw_otp_field) if raw_otp_field else None
    if otp:
        return otp, raw_otp_field
...
def after_number_inline(number, range_val):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📢 Check OTP (Channel)", url="https://t.me/TrustedScammerchat")],
        [InlineKeyboardButton("🤖 Number Bot", url="https://t.me/Fb_KiNG_Seviceotp_bot")],
