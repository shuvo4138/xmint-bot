    ...
    async def initialize(self):
        async with self.lock:
            if self.initialized:
                return
            results = []
            for i in range(27):  # 17 number + 10 OTP
                r = await self._login_once()
                results.append(r)
                await asyncio.sleep(1)
            number_count = 0
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        keyboard = InlineKeyboardMarkup([[
            ...
            InlineKeyboardButton("🤖 Number Bot", url="https://t.me/BatparaxMail_bot"),
        ]])
...
def after_number_inline(number, range_val):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📢 Check OTP (Channel)", url="https://t.me/+XsQZuKQX7LFjODU1")],
